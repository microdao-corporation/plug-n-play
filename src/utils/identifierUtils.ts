...
const getTokenIdentifier = (canisterId: string, tokenIndex: number): string => {
  ...
  // Combine the principal ID with the token index to form a token identifier
...
const wordArrayToByteArray = (
  ...
  if (
    (wordArray as crypto.lib.WordArray).hasOwnProperty('sigBytes') &&
    (wordArray as crypto.lib.WordArray).hasOwnProperty('words')
  ) {
    length = (wordArray as crypto.lib.WordArray).sigBytes;
    wordArray = (wordArray as crypto.lib.WordArray).words;
  }
  ...
  let i = 0;
  while (length > 0) {
    bytes = wordToByteArray((wordArray as number[])[i], Math.min(4, length));
    length -= bytes.length;
    result = [...result, ...bytes];
    i++;
  }
...
const getAccountIdentifier = (
  ...
  try {
    ...
    sha.update(ACCOUNT_DOMAIN_SEPARATOR); // Internally parsed with UTF-8, like go does
